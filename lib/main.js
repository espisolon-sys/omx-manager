...
/*---- Private Functions ----*/
var _stdoutFunction = function (err) {
  _this.emit('stdout', err);
};
var _stderrFunction = function (err) {
  ...
  if (!_supportsMultipleNativeLoop && (loop || _currentVideos.length > 1)) {
    ...
    respawn = function () {
      ...
      if (nextVideo) {
        ...
        _omxProcess.stdout.on('data', _stdoutFunction);      
        _omxProcess.stderr.on('data', _stderrFunction);    
